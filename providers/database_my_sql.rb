class Chef::Provider::ZabbixDatabaseMySql < Chef::Provider::LWRPBase
  ...
  action :create do
    if database_exists?(
      ...
      new_resource.host,
      new_resource.port,
      new_resource.root_username,
      new_resource.root_password)
  ...
  def database_exists?(dbname, host, port, root_username, root_password)
    ...
    db = nil
    begin
      db = ::Mysql2::Client.new(host: host, username: root_username, password: root_password, database: dbname, port: port)
      exists = true
